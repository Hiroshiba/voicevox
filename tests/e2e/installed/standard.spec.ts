...
test("標準版でエンジンをインストールして音声を書き出しWAV形式を確認できる", async () => {
  ...
  stack.defer(() => terminateProcess(appProcess));
  ...
  const welcomePage = await test.step("Welcome画面を開く", async () => {
    const page =
      context.pages()[0] ?? (await context.waitForEvent("page", { timeout }));
    await expect(
  ...
  await test.step("CPU版エンジンを選択する", async () => {
    ...
    await welcomePage.getByRole("option", { name: "CPU", exact: true }).click();
  });
  ...
  const outputDir = await test.step("保存先を設定する", async () => {
    ...
    await editorPage.evaluate(async (fixedExportDir) => {
      const savingSetting = await window.backend.getSetting("savingSetting");
